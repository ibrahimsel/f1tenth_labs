...
class reactive_follow_gap:
    ...
    def preprocess_lidar(self, ranges, angle_min, angle_increment):
        ...
        """
        self.a_idx = int((math.radians(50) - angle_min) / angle_increment)
        self.b_idx = int((math.radians(-50) - angle_min) / angle_increment)
    ...
    def safety_bubble(self, safety_bubble_radius, ranges, min_distance_idx):
        ...
        while safety_bubble_radius > -safety_bubble_radius:
            ranges[int(min_distance_idx - (safety_bubble_radius / 2))] = 0
    ...
    # TODO doesnt detect the gaps the true way. FIX THIS
    def find_max_gap(self, free_space_ranges):
        # Find max length gap
        temp = []
        max_gap = []
        max_gap_counter = -np.inf
        for i in range(self.b_idx, self.a_idx):
            if free_space_ranges[i] != 0:
                temp.append(free_space_ranges[i])
                if (len(max_gap) > max_gap_counter):
                    max_gap = temp
            else:
                temp = []
        print(max_gap)
    ...
    def find_best_point(self, max_gap, ranges):
        """Start_i & end_i are start and end indicies of max-gap range, respectively
        Return index of best point in ranges
            Naive: Choose the furthest point within ranges and go there
        """
        try:
            best_point = max(set(ranges).intersection(max_gap))
            print(best_point)
        except ValueError:
            print("HELPHELPHELP")
        return best_point
    ...
    def lidar_callback(self, data):
        ...
        proc_ranges = self.safety_bubble(safety_bubble_radius=10, ranges=proc_ranges, min_distance_idx=proc_ranges.index(min_distance))
        max_gap = self.find_max_gap(proc_ranges)
        best_point = self.find_best_point(max_gap, proc_ranges)
